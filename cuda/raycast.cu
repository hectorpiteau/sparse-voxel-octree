...
__device__ void consider_leaf_device(
  ...
  const float t0 = fmaxf(fmaxf(t_near, clip_near), 0.0f);
  const float t1 = fminf(t_far, clip_far);
  if (t1 < t0) {
    return;
  }
...
__device__ HitCandidateDevice raycast_single_device(
  ...
  if (fminf(root_t_far, clip_far) < fmaxf(fmaxf(root_t_near, clip_near), 0.0f)) {
    return best;
  }
  ...
  while (stack_size > 0) {
    ...
    for (int child_index = 0; child_index < 8; ++child_index) {
      ...
      if (fminf(t_far, clip_far) < fmaxf(fmaxf(t_near, clip_near), 0.0f)) {
        continue;
      }
...
__device__ HitCandidateDevice raycast_single_wide_device(
  ...
  while (stack_size > 0) {
    ...
    while (next_wide_dda_cell_device(dda, cell)) {
      ...
      if (fminf(cell.t_far, clip_far) < fmaxf(fmaxf(cell.t_near, clip_near), 0.0f)) {
        continue;
      }
...
__device__ HitCandidateDevice raycast_single_coarse_octree_device(
    const NodeDescriptor* nodes,
    std::size_t num_nodes,
    const std::uint32_t* leaf_payload_indices,
    std::size_t num_leaves,
    ...
    TraversalStats* stats) noexcept {
  HitCandidateDevice best;
  if (!coarse_occupancy_enabled_device(occupancy) || !finite_vec3_device(origin)) {
    return best;
  }
  bool valid_direction = false;
  const glm::vec3 normalized_direction = normalize_or_zero_device(direction, valid_direction);
  ...
  while (next_macro_dda_cell_device(dda, cell)) {
...
__device__ HitCandidateDevice raycast_single_coarse_wide_device(
    const WideNodeDescriptor* nodes,
    std::size_t num_nodes,
    const std::uint32_t* leaf_payload_indices,
    std::size_t num_leaves,
  ...
  while (next_macro_dda_cell_device(dda, cell)) {
    ...
    add_stat_device(stats != nullptr ? &stats->macro_cells_occupied : nullptr);
    add_stat_device(stats != nullptr ? &stats->macro_tree_entries : nullptr);
    best = raycast_single_wide_device(
        nodes,
        num_nodes,
        leaf_payload_indices,
        num_leaves,
...
__global__ void raycast_kernel(
  ...
  const glm::vec3 origin = origins[index];
  const glm::vec3 direction = directions[index];
  const HitCandidateDevice hit = coarse_occupancy_enabled_device(coarse_occupancy)
      ? raycast_single_coarse_octree_device(
            nodes,
            num_nodes,
            leaf_payload_indices,
            num_leaves,
...
__global__ void raycast_wide_kernel(
  ...
  const glm::vec3 direction = directions[index];
  const HitCandidateDevice hit = coarse_occupancy_enabled_device(coarse_occupancy)
      ? raycast_single_coarse_wide_device(
            nodes,
            num_nodes,
            leaf_payload_indices,
            num_leaves,
